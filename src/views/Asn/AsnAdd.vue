<template>
  ...
  <el-row class="el-row">
    <el-col :span="8" class="el-col"><div class="grid-content ep-bg-purple" >
      ...
      <el-date-picker v-model="headerData.fahuori"  style="width: 100%"
      type="date"         placeholder="发货日期"/>
    </el-form-item>
  </div></el-col>
    <el-col :span="8" class="el-col">
      <div class="grid-content ep-bg-purple-light"   > 
      <el-form-item label="预计到货日期:"  > 
      <el-date-picker v-model="headerData.yujidaohuori"  style="width: 100%"
      type="date"         placeholder="预计到货日期"/>
    </el-form-item>
  </div></el-col>       
  </el-row>
  ...
  <el-row class="el-row">
    ...
    <el-button @click="clear"><el-icon><RefreshLeft style="width: 10em; height: 10em; margin-right: 0px"  /></el-icon><span>放弃</span></el-button>
    <el-button @click="save"><el-icon><Memo style="width: 10em; height: 10em; margin-right: 0px"  /></el-icon><span>保存</span></el-button>
    <el-button @click="print('print')"><el-icon><Printer style="width: 10em; height: 10em; margin-right: 0px"  /></el-icon><span>打印</span></el-button>
      <el-button @click="print('preview')"><el-icon><View style="width: 10em; height: 10em; margin-right: 0px"  /></el-icon><span>预览打印</span></el-button>
...
<el-table-column prop="iquantity" label="本次发货数量"  width="120">
        <template #default="{ row, $index }">
												        <input v-model="row.iquantity" class="squantity-input"   @input="squantityInput(row)" type=text t_value="" o_value="" onkeypress="if(!this.value.match(/^[\+\-]?\d*?\.?\d*?$/))this.value=this.t_value;else this.t_value=this.value;if(this.value.match(/^(?:[\+\-]?\d+(?:\.\d+)?)?$/))this.o_value=this.value" onkeyup="if(!this.value.match(/^[\+\-]?\d*?\.?\d*?$/))this.value=this.t_value;else this.t_value=this.value;if(this.value.match(/^(?:[\+\-]?\d+(?:\.\d+)?)?$/))this.o_value=this.value;" onblur="if(!this.value.match(/^(?:[\+\-]?\d+(?:\.\d+)?|\.\d*?)?$/))this.value=this.o_value;else{if(this.value.match(/^\.\d+$/))this.value=0+this.value;if(this.value.match(/^\.$/))this.value=0;this.o_value=this.value}  "/>  
				</template> 
 </el-table-column>
...
data() {
    return {
      ...
      filters:{cordercode:'',cinvcode:'',cinvname:'',cinvstd:'',
      darridate:ref(''),
      cdefine28:'',
      },
      pageNum: 1,
...
.squantity-input{
		border: 1px solid #ccc; /* 这里可以定义边框的颜色和宽度 */
		  padding: 0px; /* 内边距 */
		  margin: 0px;
